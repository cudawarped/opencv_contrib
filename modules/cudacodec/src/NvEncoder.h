...
public:
    ...
    */
    virtual void EncodeFrame(std::vector<std::vector<uint8_t>>& vPacket, std::vector<uint64_t>& outputTimeStamp, NV_ENC_PIC_PARAMS* pPicParams = nullptr);
    ...
    * an encoder session.
    */
    virtual void EndEncode(std::vector<std::vector<uint8_t>>& vPacket, std::vector<uint64_t>& outputTimeStamp);
...
private:
    ...
    */
    void GetEncodedPacket(std::vector<NV_ENC_OUTPUT_PTR>& vOutputBuffer, std::vector<std::vector<uint8_t>>& vPacket, std::vector<uint64_t>& outputTimeStamp, bool bOutputDelay);
